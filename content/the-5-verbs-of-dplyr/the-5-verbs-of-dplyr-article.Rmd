...
c stands for concatenate and its how we stick multiple things together. All strings (aka words) need to have quotes around them. The second column is sex and so on.
...
Each column has a type: name and sex are <chr> which stands for characters,
...
We also could have gotten just the name and hamsters columns by removing the sex and hamster_cages columns:

```{r}
hamsters

hamsters %>% 
  select(-sex, -hamster_cages)
```
...
hamsters %>% 
  select(hamsters, hamster_cages, sex, name)
```
...
Groups can be confusing at first but they are incredibly useful. Usually code operates on rows. For example, the code above checked each row to see if the sex was female in that row.
...
We first grouped by sex. After that, every operation will happen at the group level instead of the row level.

The final line is where the magic happens. It tells R to return only the sex group where the max number of hamster cages is 3. This is the female group.

Notice that the male group isn't included because the max number of hamster cages is 4, not 3.

Similarly, we can get the sex group where the mean number of hamsters is 1.5. This time its the male group because there are two males -- one with 1 hamster and the other with 2 hamsters.
...
The n() function is a shortcut for the number of rows in the group. So, the following code finds the sex group with 3 rows in it: 
...
Of course, there isn't any reason that we have to group by sex. We could instead group by the number of hamster cages. 
...
hamsters %>% 
  group_by(sex) %>% 
  mutate(hamster_cages_centered_by_sex = hamster_cages - mean(hamster_cages))
```
...
Now because we are grouping by sex, we subtract 2 off of hamster_cages for females and 3.5 off of hamster_cages for males. This is because mean(hamster_cages) operates on groups of rows defined by sex after we add the "group_by(sex)" attribute to the data frame.
...
Of course, we don't have to group by sex (it just happens to be the most natural in this case):
...
hamsters %>%
  ...
  mutate(walruses = 0) %>% 
  group_by(sex) %>% 
  mutate(hamsters_centered_by_sex = hamsters - mean(hamsters))
...
Maybe you want the person with the fewest number of hamsters by sex:
